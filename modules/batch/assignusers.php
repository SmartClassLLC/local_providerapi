...
use local_providerapi\local\selectors\existing_batch_users;
use local_providerapi\local\selectors\potential_batch_users;
...
// Baseurl.
$baseurl = new moodle_url('/local/providerapi/modules/batch/assignusers.php',
        array('id' => $batchid, 'institutionid' => $institutionid));

$batchmemberurl = new moodle_url('/local/providerapi/modules/batch/members.php', array('id' => $batchid));
if ($returnurl) {
    $baseurl->param('returnurl', $returnurl);
    $returnurl = new moodle_url($returnurl);
} else {
    $returnurl = $batchmemberurl;
}
...
$viewnode =
        $node->add(get_string('batchmembers', 'local_providerapi'), $batchmemberurl, navigation_node::TYPE_SETTING, null,
                'viewmembers');
...
$potentialuserselector = new potential_batch_users('addselect', array('institutionid' => $institutionid, 'batchid' => $batchid));
$existinguserselector = new existing_batch_users('removeselect', array('batchid' => $batchid));
...
if (optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoremove)) {
        foreach ($userstoremove as $removeuser) {
            $batch->remove_member($removeuser->id);
        }
        $potentialuserselector->invalidate_selected_users();
        $existinguserselector->invalidate_selected_users();
    }
