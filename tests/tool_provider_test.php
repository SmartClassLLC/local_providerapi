 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_providerapi_tool_provider_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        $toolrecord = $providergenerator->create_lti_tool();
    ...
    public function test_on_error() {
        $tool = $this->tool;
        $tp = new dummy_local_api_tool_provider($tool->id);
        $message = "THIS IS AN ERROR!";
        $tp->message = $message;
        $tp->onError();
        $errormessage = get_string('failedrequest', 'local_providerapi', ['reason' => $message]);
    ...
    public function test_on_register_no_consumer() {
        ...
        $this->assertFalse($tp->ok);
        $this->assertEquals(get_string('invalidtoolconsumer', 'local_providerapi'), $tp->message);
    }
    ...
    public function test_on_register_no_return_url() {
        ...
        $tp->onRegister();
        $this->assertFalse($tp->ok);
        $this->assertEquals(get_string('returnurlnotset', 'local_providerapi'), $tp->message);
    }
    ...
    public function test_on_register_failed() {
        ...
        $dataconnector = new data_connector();
        $consumer = new dummy_local_api_tool_consumer('testkey', $dataconnector);
        $consumer->ltiVersion = ToolProvider::LTI_VERSION2;
        $consumer->secret = $tool->secret;
        $consumer->name = 'TEST CONSUMER NAME';
        $consumer->consumerName = 'TEST CONSUMER INSTANCE NAME';
        ...
        $this->assertFalse($tp->ok);
        // Check message.
        $this->assertEquals(get_string('couldnotestablishproxy', 'local_providerapi'), $tp->message);
    }
    ...
    public function test_on_register() {
        ...
        $successmessage = get_string('successfulregistration', 'local_providerapi');
        ...
        $this->assertTrue($DB->record_exists('local_api_tool_consumer_map', $mappingparams));
    }
    ...
    public function test_on_launch_no_frame_embedding() {
        ...
        $this->assertContains(get_string('frameembeddingnotenabled', 'local_providerapi'), $output);
    }
    ...
    public function test_on_launch_with_invalid_secret_and_no_proxy() {
        ...
        @$tp->onLaunch();
        $this->assertFalse($tp->ok);
        $this->assertEquals(get_string('invalidrequest', 'local_providerapi'), $tp->message);
    }
                        ...
                        'resource_handler' => [
                                [
                                        'message' => [
                                                [
                                                        'message_type' => 'basic-lti-launch-request',
                                                        'path' => '/local/providerapi/tool.php?id=' . $tool->id
                                                ]
                                        ]
                                ]
                        ]
    ...
    public function test_on_launch_with_user_image_from_resource_link() {
        ...
        $user = $DB->get_record('user', ['id' => $tp->user->ltiUserId]);
        // User was found.
    ...
    public function test_on_launch_no_consumer() {
        ...
        $this->assertFalse($DB->record_exists('local_api_tool_consumer_map', $mappingparams));
    }
    ...
    public function test_map_tool_to_consumer_no_consumer() {
        $tp = new dummy_local_api_tool_provider($this->tool->id);
        $this->expectException('moodle_exception');
        $tp->map_tool_to_consumer();
    }
     ...
     * @return dummy_local_api_tool_provider
     */
    protected function build_dummy_tp($secret = null, $proxy = null, $resourcelinksettings = null) {
        ...
        $ltiuser = User::fromResourceLink($resourcelink, $user->id);
        $ltiuser->ltiResultSourcedId = 'testLtiResultSourcedId';
        $ltiuser->email = 'user1@example.com';
        $ltiuser->save();

        $this->getDataGenerator()->enrol_user($user->id, $this->tool->courseid, 5);
...
/**
 * Class dummy_local_api_tool_provider.
 *
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dummy_local_api_tool_provider extends tool_provider {
...
/**
 * Class dummy_local_api_tool_consumer
 *
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dummy_local_api_tool_consumer extends ToolConsumer {
    ...
    /**
     * dummy_local_api_tool_consumer constructor.
     *
