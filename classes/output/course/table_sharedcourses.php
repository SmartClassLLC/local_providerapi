 ...
 */
class table_sharedcourses extends table_sql implements \renderable {
    ...
    public function __construct($baseurl, $pagesize) {
        ...
        $this->sortable(true, 'coursefullname');
        $this->pageable(true);
    ...
    public function col_manage($row) {
        ...
        if (has_capability('local/providerapi:deletesharedcourse', $this->context)) {
            ...
            $buttons[] = $OUTPUT->action_icon($deleteurl, $visibleimg,
                    new confirm_action(get_string('areyousuredel', 'local_providerapi', $row->coursefullname)));
        }
