 ...
 */
class potential_batch_users extends user_selector_base {
    ...
    public function find_users($search) {
        ...
        list($select, $from, $where, $instparams) = $institution->get_member_sql();
        $whereconditions[] = $where;
        $params = array_merge($params, $instparams);
        list($select, $batchfrom, $batchwhere, $batchparams) = $batch->get_member_sql();
        list($wherecondition, $searchparams) = $this->search_sql($search, 'u');
        if ($wherecondition) {
            $whereconditions[] = $wherecondition;
            $params = array_merge($params, $searchparams);
        }
        $userids = $DB->get_fieldset_sql("SELECT u.id FROM {$batchfrom} WHERE {$batchwhere} ", $batchparams);
        if ($userids) {
    ...
    protected function get_options() {
        ...
        $options['file'] = 'local/providerapi/classes/local/selectors/potential_batch_users.php';
        return $options;
    }
}